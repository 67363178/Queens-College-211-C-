...
int main(){
 ...
 // odd and even have different starting position
if((n%2)!=0){
from=0;//which tower move from
to=1;//which tower move to
candidate=1;
}
else{
from=0;
...
while( t[1].size()<n+1){ // there are still rings to transfer to tower B = t[1]
 //write it
 cout<<"move number "<<++move<<": Transfer ring" <<candidate<<" from "<< char(from+65)<<" to "<<char(to+65)<<endl;
